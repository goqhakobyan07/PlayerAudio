...
function aniamte() {
  setTimeout(() => {
    allAudio.forEach((item,i) => {
      ...
      item.style.transitionDelay = i  / 10 * .1 + 's'
    });
  }, 100);
}
...
function list(arrayname) {
  arrayname.forEach((item, i) => {
    ...
    actualPlaylist.append(musicItem);
    ...
    musicItem.onclick = () => {
      playlist.classList.remove('openplaylist')
